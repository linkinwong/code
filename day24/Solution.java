...
public class Solution {
  ...
  public int maxProfit(int[] prices) {
    int MAX=2, max_diff=0, len = prices.length;
    int[] f = new int[MAX, len];
    for(int i=1;i<MAX; i++){
      max_diff = f[i-1][0] - prices[0];
      for(int j=1; j<len; j++){
        f[i][j] = Math.max(f[i][j-1], max_diff + prices[j]);
        max_diff = Math.max(max_diff, f[i-1][j] - prices[j]);
      }
    }
    return f[MAX-1][len-1];
  }
